...
## 2026-08-18: Booting the full API concurrently with `precompute_similar_orgs.py` pushed the box into swap-thrashing, and a worker got silently OOM-killed

**Symptom:** While a `safe_deploy_droplet.sh --force` rebuild was 52% through Pass 2 (6 `precompute_similar_orgs.py` workers, each holding ~4.4GB RSS of "slim org" data), load average spiked from a normal single digits to **304** within a couple of minutes, `free -h` showed swap at 8.0Gi/8.0Gi (100% full) with under 500Mi RAM free, and the rebuild's own progress log went silent for 19+ minutes (last line stuck at 900,000/1,717,160). A new `precompute_similar_orgs.py` process appeared with a fresh start time, which on first glance looked like a second, independent job — `ps -o ppid` showed it was actually a child of the *same* driver PID as the original workers, which ruled that out.

**Root cause:** A smoke test kicked off in a separate terminal (`python3 -c "import daanaa_api as api; ... client = app.test_client()"`) booted the full Flask app, which eagerly loads ~546K org embedding vectors into RAM (~7.4GB, by design — see `_load_embeddings()` / gunicorn `--preload`). Starting that at the same time as 6 already-heavy precompute workers (already close to the 30GB ceiling) tipped the box into swap. The Linux OOM killer took out at least one process under that pressure; Python's `multiprocessing.Pool` (no `maxtasksperchild` set) auto-replaces a worker that dies unexpectedly, which is exactly why a new worker PID appeared mid-run — it was the pool healing itself, not a duplicate job. The two looked identical from a first `ps aux` glance (same script name, a fresh start time); only checking the parent PID told them apart.

**Preventing rule:**

> Before starting anything that eagerly loads a large in-memory structure (the embedding matrix, a big precompute run, a full gunicorn boot with `--preload`) — even a "just a quick smoke test" — check `free -h` and `ps aux --sort=-%cpu` first if any other data-pipeline job might already be running. `check_merit_status.sh` should be the reflexive first move, not an afterthought, whenever a heavy background job's start time is unknown. And when a duplicate-looking process shows up mid-incident: check `ps -o ppid` before assuming it's a second overlapping job (another session, a stray cron, a retry) — a `multiprocessing.Pool` with no `maxtasksperchild` silently replaces dead workers with same-named, freshly-started processes, which looks identical to a real duplicate at first glance and led to an initially wrong diagnosis mid-incident here.

**Fixed:** Killed the stuck smoke-test process (already OOM-killed by the time the kill command ran); did not touch any `precompute_similar_orgs.py` worker. Memory pressure eased within ~2 minutes (1-min load average 304 → 17), and the rebuild resumed forward progress (900K → 1,000,000 orgs) on its own. No process was killed that shouldn't have been; the near-miss was almost killing the pool's legitimate replacement worker before checking its parent PID.

---

## 2026-08-18: A fail-closed deploy gate silently verified zero rows, on every run, for its entire life

**Symptom:** `scripts/testing/validate_link_integrity.py` — the pre-ship gate proving precompute donate/website links match the source snapshot exactly — printed "Checking 97360 orgs with donate_url (exhaustive)..." followed by "Donate: 0 verified, 0 missing-file" and exited 0 with "✓ LINK INTEGRITY OK". An exhaustive check landing on 0+0 out of 97,360 doesn't add up (it should land on *some* combination of verified/missing that sums close to the input count), and was only caught because the output was actually read rather than trusted on a green exit code.

**Root cause, two independent bugs found in sequence by fixing one at a time and re-running:**
1. `registry_enriched.deductibility` is stored as **TEXT** (`'1'`, `'2'`, `'0'`, `'revoked'` — confirmed via `typeof(deductibility)`), but the script's Python-side filter compared it against the **int** `1` (`if r["deductibility"] != 1: continue`). `'1' != 1` is always `True` in Python, so every one of the 97,360 donate_url rows hit `continue` before either counter could increment. `scripts/registry_filters.py:27` already spells this same predicate correctly (`deductibility = '1'`, quoted) — this script re-spelled the predicate inline instead of importing the canonical one, and got the type wrong.
2. Fixing just the type (`!= "1"`) surfaced a second bug: 3,078 "failures" appeared, all `org_status IN ('revoked','inactive')` — orgs `precompute_orgs.py` correctly excludes from its output (`deductibility = 1 AND org_status = 'active'`, precompute_orgs.py:260/291) but that this script's hand-rolled predicate didn't. Same root cause as bug 1: re-spelling a predicate inline instead of importing `DEDUCTIBLE_FILTER`, whose own docstring literally says "Do not re-spell this predicate inline anywhere else — import it" and lists two prior incidents caused by exactly that.

**Fixed:** Rewrote both SQL queries (donate check and website check) to filter with `{DEDUCTIBLE_FILTER}` imported from `scripts/registry_filters.py`, instead of hand-rolled predicates. Re-running against the same snapshot went 0+0 → 3,078 failures → **5** failures (all `org_status='inactive'`, a status `DEDUCTIBLE_FILTER` itself doesn't exclude — a separate, much smaller, pre-existing population-boundary question between the "public count" filter and precompute's stricter "active only" filter, not a corruption issue; matches an already-documented 6-EIN precedent in `DECISIONS.md` 2026-08-15). Zero actual donate_url/website *mismatches* found across 89,061 verified orgs — the substantive stewardship guarantee (no corrupted links) holds; this gate had simply never been checking it before tonight.

**Preventing rule:**

> A "0 and 0" (or any suspiciously round/empty) result from an "exhaustive" check is not evidence of a clean pass — it's evidence the check didn't run. Before trusting a gate's green exit code, sanity-check that its own reported numbers add up to something close to its stated input size. And: this is the third incident in this file caused by re-spelling a predicate inline instead of importing the file that says, in its own docstring, not to — `registry_filters.py` existed specifically to prevent this, and the guidance was still not followed by code written after it existed. When a canonical single-source-of-truth predicate/filter file exists, grep for it before writing a new `WHERE` clause that looks similar to what it already defines.

Not yet deployed to the droplet — precompute rebuild (interrupted by a separate stuck-worker incident earlier the same day, see the entry above) was still completing when this was found; shipping decision pending.

---

## 2026-08-18: `deploy_droplet.sh` has referenced a systemd unit that has never existed

**Symptom:** Running the atomic-swap deploy step, Steps 1-4 (checksum, extract, health-check, swap) all succeeded, but Step 5 ("Restarting API") failed: `Failed to start daanaa.service: Unit daanaa.service not found`. The earlier "Stopping API service" step in Step 4 had also silently failed the same way (`Unit daanaa.service not loaded`) without aborting the script (both calls use `|| true`).

**Root cause:** `scripts/ops/deploy_droplet.sh:14` sets `API_SERVICE="daanaa"`, but the real, actual systemd unit running the API on the droplet — confirmed live via `systemctl list-units`, and referenced correctly by name in this same night's `daanaa-api` crash-loop lesson (2026-08-12) and outage lesson (earlier this entry's neighbor) — is `daanaa-api.service`. `systemctl stop/start daanaa` expands to `daanaa.service`, a unit that has never existed on this droplet. This bug is in the repo's own tracked copy of the script (confirmed identical on the droplet's live copy too), not a one-off drift — every previous deploy through this script must have hit the identical silent no-op on both stop and restart.

**Why it didn't cause an outage:** both the stop and start calls fail closed in the "doesn't do anything" direction (a nonexistent unit can't be stopped or started, so the already-running process was simply never touched), rather than in a way that kills a real process. The atomic *data* swap (Step 4) is entirely independent of the service commands and completed correctly regardless. Net effect: the deploy silently never restarted the API on any prior run through this exact path, serving stale precompute data indefinitely after each such "successful" deploy, without erroring loudly enough to be caught (`|| true` on both systemctl calls, and the script's own overall exit code doesn't reflect this — only the final restart's `exit 5` this time actually surfaced it, because that call isn't guarded with `|| true`).
