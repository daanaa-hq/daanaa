    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-warm-cream px-6">
        <div className="max-w-md text-center bg-white p-8 rounded-2xl shadow-sm">
          ...
          <p className="font-body text-[14px] text-cool-grey">{error}</p>
        </div>
      </div>
    )
  }
              ...
              return (
                <div key={a.ein} className="flex items-center justify-between gap-3 p-3 bg-light-grey/30 rounded-xl">
                  ...
                  <button
                    ...
                    disabled={already}
                    className="shrink-0 px-3 py-1.5 rounded-lg bg-soft-gold text-deep-navy font-body text-[12px] font-semibold hover:bg-bright-gold disabled:opacity-50 disabled:bg-emerald-100 disabled:text-emerald-700 transition-colors"
                  >
    ...
    <div className="min-h-screen bg-warm-cream">
      <div className="bg-deep-navy py-8 px-6">
        <div className="max-w-[560px] mx-auto">
          <h1 className="font-display italic text-warm-cream text-[32px] leading-tight">{info?.title}</h1>
          <p className="font-body text-muted-cream mt-2 text-[14px]">
            {info?.event_date}
      ...
      <div className="max-w-[560px] mx-auto px-6 py-10">
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block font-body text-[12px] font-semibold text-deep-navy mb-1">Your name</label>
              <input value={name} onChange={e => setName(e.target.value)}
                className="w-full px-3 py-2.5 border border-light-grey rounded-lg font-body text-[14px]" required />
            </div>
            <div>
              <label className="block font-body text-[12px] font-semibold text-deep-navy mb-1">Your email</label>
          ...
          <div className="space-y-3">
            <label className="block font-body text-[12px] font-semibold text-deep-navy">
                  ...
                  {info && info.orgs.length > 1 && (
                    <label className="flex items-center gap-2 mb-3 cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4"
                      />
                      <span className="font-body text-[14px] font-semibold text-deep-navy">{org?.name}</span>
                    </label>
                  )}
                  {a.checked && (
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block font-body text-[11px] text-cool-grey mb-1">Hours</label>
                        <input
                      ...
                      <div>
                        <label className="block font-body text-[11px] text-cool-grey mb-1">Role</label>
                        <select
                          value={a.task_type}
          ...
          <div>
            <label className="block font-body text-[12px] font-semibold text-deep-navy mb-1">Notes (optional)</label>
          ...
          <button type="submit" disabled={submitting}
            className="w-full py-3 rounded-xl bg-soft-gold text-deep-navy font-body text-[14px] font-semibold hover:bg-bright-gold disabled:opacity-50 transition-colors">
