...
export default function NonprofitVerification() {
  ...
  useEffect(() => {
    ...
    const fetchHours = async () => {
      try {
        setLoading(true)
        setError(null)
        const response = await fetch('/api/nonprofit/hours-pending')
  ...
  const handleVerify = async (recordId: string) => {
    if (!verificationMessage.trim()) {
      alert('Please add a verification note')
      return
    }
    ...
    try {
      ...
      alert('Error verifying hours: ' + (err instanceof Error ? err.message : 'Unknown error'))
    } finally {
      setVerifying(false)
    }
  }
  ...
  const handleReject = async (recordId: string) => {
    ...
    try {
      ...
      alert('Error rejecting hours: ' + (err instanceof Error ? err.message : 'Unknown error'))
    } finally {
      setVerifying(false)
    }
  }
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-12">
        ...
        <div className="flex gap-2 mb-8">
          {(['all', 'pending', 'verified', 'rejected'] as const).map((status) => (
            ...
            >
              ...
              {' '}
              <span className="text-sm">({hours.filter((h) => status === 'all' || h.status === status).length})</span>
            </button>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {filteredHours.length === 0 ? (
            <div className="bg-white border border-light-grey rounded-lg p-8 text-center">
            ...
            filteredHours.map((record) => (
              <div
                key={record.id}
                className="bg-white border border-light-grey rounded-lg p-6 cursor-pointer hover:shadow-md transition-shadow"
                onClick={() => setSelectedHours(record.id === selectedHours ? null : record.id)}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <p className="font-semibold text-deep-navy mb-1">{record.volunteer_email}</p>
                    <p className="text-sm text-cool-grey mb-2">{record.service_date}</p>
                    <div className="flex gap-4 text-sm">
                      <span className="font-medium text-deep-navy">{record.hours_logged} hours</span>
                      {record.notes && <span className="text-cool-grey italic">"{record.notes}"</span>}
                    </div>
                  </div>
                    ...
                    {record.status === 'pending' && (
                      <>
                        ...
                        <div className="flex gap-3">
                          ...
                          <button
                            onClick={() => {
                              const reason = prompt('Why are you rejecting these hours?')
                              if (reason) {
        ...
        {hours.length > 0 && (
          <div className="mt-12 grid grid-cols-3 gap-4">
            <div className="bg-white border border-light-grey rounded-lg p-4 text-center">
              <p className="text-sm text-cool-grey mb-1">Pending</p>
              <p className="text-2xl font-display text-deep-navy">{hours.filter((h) => h.status === 'pending').length}</p>
            </div>
            <div className="bg-white border border-light-grey rounded-lg p-4 text-center">
              <p className="text-sm text-cool-grey mb-1">Verified</p>
              <p className="text-2xl font-display text-green-600">{hours.filter((h) => h.status === 'verified').length}</p>
            </div>
            <div className="bg-white border border-light-grey rounded-lg p-4 text-center">
              <p className="text-sm text-cool-grey mb-1">Rejected</p>
