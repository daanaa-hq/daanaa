    ...
    return (
      <div className="min-h-screen bg-warm-cream px-4 py-16">
        <div className="max-w-2xl mx-auto text-center">
          <h1 className="font-display italic text-deep-navy text-headline mb-4">Partner not found</h1>
    ...
    <div className="min-h-screen bg-warm-cream">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <section className="mb-12">
          <h2 className="font-display italic text-deep-navy text-title-lg mb-8">Benefits</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {Object.entries(TIER_INFO).map(([tier, info]) => (
              <div key={tier} className={`${info.color} rounded-2xl p-6 border border-light-grey`}>
                <h3 className="font-semibold text-lead uppercase tracking-wide mb-4">{info.label}</h3>
                <ul className="space-y-3">
                  {guild.benefits[tier as keyof typeof guild.benefits].map((benefit, idx) => (
                    <li key={idx} className="flex gap-2 text-small">
                      <span className="shrink-0">✓</span>
                      <div>
                        <div className="font-semibold">{benefit.feature_name}</div>
                        {benefit.description && <div className="text-caption opacity-75">{benefit.description}</div>}
                      </div>
                    </li>
                  ))}
                </ul>
        ...
        <section>
          <h2 className="font-display italic text-deep-navy text-title-lg mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {guild.members.map((org) => (
                ...
                >
                  ...
                  {(org.city || org.state) && (
                    <p className="text-small text-cool-grey">
