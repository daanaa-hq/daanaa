...
function CreateEventForm({ ein, idToken, onCreated }: { ein: string; idToken: string; onCreated: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-light-grey p-6 space-y-4">
      <h3 className="font-body text-body-lg font-semibold text-deep-navy">New volunteer event</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block font-body text-caption font-semibold text-deep-navy mb-1">Event name</label>
        ...
        <div>
          <label className="block font-body text-caption font-semibold text-deep-navy mb-1">Date</label>
          <input type="date" value={eventDate} onChange={e => setEventDate(e.target.value)}
            className="w-full px-3 py-2 border border-light-grey rounded-lg font-body text-body" />
        </div>
        <div>
          <label className="block font-body text-caption font-semibold text-deep-navy mb-1">City</label>
          <input value={city} onChange={e => setCity(e.target.value)} placeholder="Austin"
            className="w-full px-3 py-2 border border-light-grey rounded-lg font-body text-body" />
        </div>
        <div>
          <label className="block font-body text-caption font-semibold text-deep-navy mb-1">State</label>
          <input value={state} onChange={e => setState(e.target.value)} placeholder="TX" maxLength={2}
            className="w-full px-3 py-2 border border-light-grey rounded-lg font-body text-body" />
        </div>
        <div className="md:col-span-2">
          <label className="block font-body text-caption font-semibold text-deep-navy mb-1">
          ...
          <input value={coOrgEins} onChange={e => setCoOrgEins(e.target.value)} placeholder="123456789, 987654321"
            className="w-full px-3 py-2 border border-light-grey rounded-lg font-body text-body" />
          <p className="font-body text-label text-cool-grey mt-1">
...
function ImportEventForm({ ein, idToken, onCreated }: { ein: string; idToken: string; onCreated: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="bg-white rounded-2xl border border-soft-gold/30 p-6 space-y-3">
      <h3 className="font-body text-body-lg font-semibold text-deep-navy">Build from a public event link</h3>
      <p className="font-body text-caption text-cool-grey">Daanaa will create an AI assisted, unconfirmed draft for your review.</p>
      {error && <div className="p-3 bg-destructive/5 rounded-lg text-destructive font-body text-small">{error}</div>}
      <input
        type="url"
        required
        value={url}
  ...
  return (
    <div className="bg-white rounded-2xl border border-light-grey p-6">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <h3 className="font-body text-body-lg font-semibold text-deep-navy">{event.title}</h3>
        ...
        <span className={`px-2.5 py-0.5 rounded-full font-body text-label font-semibold border ${
      ...
      {shortUrl && (
        <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t border-light-grey">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-body text-caption text-cool-grey mb-1">Volunteer hour-logging link</p>
            <div className="flex items-center gap-2">
              <code className="font-body text-caption bg-light-grey/40 px-2 py-1 rounded truncate">{shortUrl}</code>
              <Button onClick={copy} variant="secondary" size="sm" className="shrink-0">
                {copied ? 'Copied!' : 'Copy'}
              </Button>
            </div>
            <p className="font-body text-label text-cool-grey mt-2">
    ...
    <div className="min-h-screen bg-warm-cream">
      <div className="bg-deep-navy py-8 px-6">
        <div className="max-w-[900px] mx-auto">
          <Link to="/nonprofit/my-orgs" className="font-body text-small text-muted-cream hover:text-warm-cream">← My organizations</Link>
          <h1 className="font-display italic text-warm-cream mt-3 h1-display">Volunteer Events</h1>
          <p className="font-body text-muted-cream mt-2 text-body-lg">
      ...
      <div className="max-w-[900px] mx-auto px-6 py-10 space-y-6">
        <div className="flex justify-between items-center">
          <Link to={`/nonprofit/impact/${ein}`} className="font-body text-small text-link-gold hover:underline">
            View yearly impact →
          </Link>
        </div>
        ...
        {loading ? (
          <p className="font-body text-cool-grey text-body">Loading events...</p>
        ) : events.length === 0 ? (
          <p className="font-body text-cool-grey text-body">No events yet. Create one above to get a QR code volunteers can scan to log hours.</p>
        ) : (
