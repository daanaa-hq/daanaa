...
export default function TrustNav() {
  ...
  return (
    <div className="border-t border-light-grey pt-10 mt-16">
      <p className="font-body text-label tracking-[0.1em] text-link-gold uppercase mb-5">Continue exploring Daanaa</p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {PAGES.map(({ to, label, sub }) => {
          ...
          return active ? null : (
            ...
            >
              <div>
                <p className="font-body text-label tracking-[0.06em] text-cool-grey uppercase mb-0.5">{sub}</p>
                <p className="font-display italic text-deep-navy text-lead leading-snug">{label}</p>
              </div>
