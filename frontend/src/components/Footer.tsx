  ...
  return (
    <footer className="bg-deep-navy border-t border-navy-mid">
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-12 pt-12 pb-8">
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-8">
          ...
          <div className="col-span-2 lg:col-span-1">
            <Link to="/" className="font-display italic text-title text-warm-cream tracking-[-0.02em]">
              Daanaa
            </Link>
            <p className="mt-2 font-body text-body leading-[1.5] tracking-[0.01em] text-muted-cream">
          ...
          {NAV_COLS.map(({ heading, links }) => (
            <div key={heading}>
              <p className="font-body text-caption font-medium tracking-[0.08em] text-pale-gold uppercase mb-3">
                {heading}
              </p>
                ...
                {links.map(({ label, to, href }: { label: string; to?: string; href?: string }) => (
                  <li key={label}>
                    {to ? (
                      <Link
                        to={to}
                        className="font-body text-body text-muted-cream hover:text-warm-cream transition-colors duration-150 block py-1.5"
                      >
                        {label}
                      </Link>
                    ) : (
      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-12 py-6 border-t border-navy-mid space-y-2">
        <p className="font-body text-small tracking-[0.02em] text-muted-cream pb-2">
        ...
        <p className="font-body text-caption tracking-[0.02em] text-muted-cream">
