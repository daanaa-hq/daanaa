...
#
# Fixed 2026-08-21: both subprocess calls below referenced stale
# pre-folder-migration paths (scripts/sync_irs_data.py and
# scripts/search_index_delta.py -- the files live at scripts/migrations/
# and scripts/search/ respectively). This silently broke the weekly sync
# starting 2026-08-17 (set -e meant the first broken path killed the job
# before the second broken path was ever reached) -- found while checking
# whether new IRS data had come in since the last successful run
# (2026-08-10, +0 new orgs). See LESSONS.md 2026-08-21.
...
python3 scripts/migrations/sync_irs_data.py --mode delta --log-file "$LOG_FILE" 2>&1 || {
...
python3 scripts/search/search_index_delta.py 2>&1 | tee -a "$LOG_FILE" || {
