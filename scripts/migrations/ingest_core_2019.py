...
for name, path in files.items():
    ...
    df = pl.read_csv(
        ...
        dtypes={c: pl.Utf8 for c in cols_to_read},
        low_memory=True,
        n_threads=8
    )
...
def safe_str(col, expr):
    """Apply string expression only on non-null values."""
...
def clean_df(df, source_tag):
    ...
    # EIN: remove non-digits, pad to 9
    df = df.with_columns(
    ...
    # REVENUE / ASSETS: clean and cast to int
...
deduped.to_pandas().to_sql("core_2019_staging", conn, if_exists="append", index=False)
print(f"    Written {after:,} rows to core_2019_staging")
