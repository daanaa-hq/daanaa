...
for name, path in files.items():
    ...
    df = pl.read_csv(
        ...
        schema_overrides={c: pl.Utf8 for c in cols_to_read},  # fixed: schema_overrides
        low_memory=True,
        n_threads=8
    )
...
# ============================================================================
# 5. WRITE TO SQLITE (direct inserts, no pandas to_sql lock)
# ============================================================================
print("\n" + "=" * 60)
...
# Bulk insert via executemany (no pandas lock conflict)
pdf = deduped.to_pandas()
