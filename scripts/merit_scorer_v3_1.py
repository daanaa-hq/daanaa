#!/usr/bin/env python3
"""
MERIT Score Engine v3.1 — Fixed peer grouping (1-char NTEE major + revenue band)
"""
...
def get_peer_key(ein, ntee_lookup, revenue):
    """Primary peer group: NTEE 1-char major + revenue band."""
...
def calculate_merit(ein, org, ntee_lookup, peer_groups):
    ...
    if len(peers) < 5:
        # Fallback: revenue band only (cross-category comparison)
        try:
...
def main():
    ...
    for ein, org in xml_data.items():
        if ntee_lookup.get(ein):
            scorable[ein] = org
    print(f"Scorable orgs: {len(scorable)}")
    ...
    # Show largest groups
    print("\nTop 10 peer groups by size:")
    for key, members in sorted(peer_groups.items(), key=lambda x: -len(x[1]))[:10]:
        print(f"  {key}: {len(members)} orgs")
