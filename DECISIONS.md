...
invented its own sizes. 2,038 hand-rolled `text-[Npx]` across 24 distinct values,
including 16/17/18 and 24/26/28 in production. Separately, `open-data.html` (the
visibility/data page, generated by `generate_visibility_exports.py`) hand-copied
the entire palette into its own `:root` and picked its own sizes.

**Chose:** One statically-served token file, `frontend/public/tokens.css`, read by
every surface. Tailwind references it via `var(--text-body, 14px)`; standalone
pages link it directly.

**Why var() with a px fallback:** a single source of truth that standalone HTML can
also consume. Tailwind alone could not serve `open-data.html`, and duplicating the
scale into that page is what caused the drift originally. The px fallback means a
failure to load tokens.css degrades to the correct size rather than collapsing
every text element on the site.

**Why size-only tokens (no paired line-height):** makes 2,236 of the 2,326 changes
pure renames with zero visual change. Existing `leading-[...]` classes stay
authoritative. Only 90 drift sizes (8/9/17/19/22/26/30/34/42/48/52px) fold to a
nearest neighbour and actually change rendering.

**Rejected:**
- Collapsing to ~8 tokens: correct destination, but changes 700+ occurrences
  visually. Needs review, deferred as a separate step.
- Tailwind-only scale: cannot reach standalone or generated pages, which is the
  exact gap that produced the drift.
- Leaving open-data.html alone: it is a real user-facing surface and would have
  continued diverging.

**Enforcement:** `scripts/lint_type_scale.sh`, wired in as check 5 of
`frontend/scripts/design-lint.sh`. Covers raw `text-[Npx]` in TSX, raw
`font-size: Npx` in standalone HTML, and asserts standalone pages link tokens.css.
Without enforcement the scale drifts again; that is how it got here.

**Sequencing note:** the Design System v2 brief lists typography as section 5.
Measurement said it was the bottleneck: colors were already centralized in
tailwind.config.js and themes were already 60 override blocks deep, while type had
no scale at all. Ran it first.

**Verified:** 225 tests pass, build clean, tokens.css served and `var()` resolving
in preview, zero raw sizes repo-wide.

**Also:** `/design-system` skill added at `.claude/skills/design-system/SKILL.md`
documenting tokens, rules, pre-ship checks, and known remaining debt.
